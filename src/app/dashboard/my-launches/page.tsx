...
export default function MyLaunchesPage() {
  ...
  const { myprojects, mylaunchInfo } = useSelector(
    (state: { myprojects: any; mylaunchInfo: any }) => state || {},
  );
  ...
  const getLaunches = useCallback(async () => {
    ...
    const projects = res.map((p: any) => {
      ...
      const participate = p.allocation.participants.find(
        (f: any) => f.eoa === user.uid,
      );
  ...
  useEffect(() => {
    ...
    if (sortValue === "Name") {
      ...
      filtered.sort((p1: any, p2: any) => {
        return (
          p1.contribution?.formattedAmount ??
          0 > p2.contribution?.formattedAmount ??
          0
        );
      });
      ...
      filtered.sort((p1: any, p2: any) => {
        return (
          new Date(p1.pledgeEndDate).getTime() -
          new Date(p2.pledgeEndDate).getTime()
        );
      });
      ...
      {filteredProjects?.length > 0 ? (
            ...
            <div className="flex justify-between items-center px-6 py-4 flex-wrap">
              ...
              /> */}
                <div className="flex items-center gap-2 bg-transparent !text-sm !text-[#448AFF] font-bold px-2">
                  <label>Sort by:</label>
                  <select
                    className="bg-[#1b1e29] focus:outline-none cursor-pointer"
                    value={sortValue}
                    onChange={(e) => {
                  ...
                  >
                    {Object.keys(columns)
                      ...
                      .map((col) => (
                        <option
                          className="text-[#C7CAD9] cursor-pointer"
                          value={col}
                        >
                          {col}
                        </option>
                    ...
                    {filteredProjects &&
                      filteredProjects.map((row, index) => (
                        ...
                        >
                              ...
                              <div className="flex justify-start">
                                <a href="#">
                                  <small className="text-sm">
                                    {" "}
                                    {row.contribution
                                      ? `$${row.contribution.formattedAmount}`
                                      : "-"}
                                  </small>
                                </a>
                              </div>
                          ...
                          {columns["Launch Phase"] && (
                            <td className="py-1.5 px-4 min-h-14">
                              <div className="flex flex-col justify-start">
                                {row.tableData.phase.map((p: String) => (
                            ...
                            <Button
                              className={clsx({
                                ...
                                ["!bg-[#448AFF14] !text-[#448AFF]"]:
                                  row.tableData.btn === "View Details",
                              })}
                              onClick={() =>
                                router.push(
                                  `/dashboard/launch-info/${row.pid}?status=${row.status}`,
                                )
                              }
                            >
                              {row.tableData.btn}
        ...
        <div className="mx-6 my-24 flex flex-col items-center justify-center">
          <Image
            ...
            height={128}
          />
          <p className="text-[#696C80] text-lg font-bold pt-10 pb-2">
            Looks Like You Haven't Joined A Launch Yet!
          </p>
          <p className="text-[#696C80] text-sm ">
            No problem, you can check our Live and Upcoming Launches for
            projects that might interest you.
          </p>
          <Button
            variant="primary"
            className="text-bold text-sm mt-6 !h-9"
