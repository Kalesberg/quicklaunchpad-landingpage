...
const LaunchNotice: React.FC<{ status?: string }> = ({ status }) => {
  ...
  const { upcomingProject } = useSelector(
    (state: { upcomingProject: Project }) => state || {},
  );
      ...
      <div className="rounded-2xl p-8 mt-[30px] mb-[25px] relative overflow-hidden">
        <div className="relative z-10">
          ...
          <div className="flex space-x-5 mt-2 mb-4">
            {upcomingProject.socials?.telegram && (
              <a href={upcomingProject.socials.telegram} target="_blank">
                <TelegramIcon className="w-4 h-4 text-gray-400 hover:text-white cursor-pointer" />
              </a>
            )}
            {upcomingProject.socials?.youtube && (
              <a href={upcomingProject.socials.youtube} target="_blank">
                <YoutubeIcon className="w-4 h-4 text-gray-400 hover:text-white cursor-pointer" />
              </a>
            )}
            {upcomingProject.socials?.discord && (
              <a href={upcomingProject.socials.discord} target="_blank">
                <DiscordIcon className="w-4 h-4 text-gray-400 hover:text-white cursor-pointer" />
              </a>
            )}
            {upcomingProject.socials?.github && (
              <a href={upcomingProject.socials.github} target="_blank">
                <GithubIcon className="w-4 h-4 text-gray-400 hover:text-white cursor-pointer" />
              </a>
            )}
            {upcomingProject.socials?.twitter && (
              <a href={upcomingProject.socials.twitter} target="_blank">
          ...
          <Button
            onClick={() =>
              router.push(
                `/dashboard/launch-info/${upcomingProject.pid}?status=${upcomingProject.status}`,
              )
            }
            variant="secondary"
            size="medium"
